...
impl<T: 'static + std::marker::Send, D: 'static + std::marker::Send> ReflexiveQueue<T, D> {
    ...
    where F: Fn(&Receiver<T>, Sender<T>, Option<Sender<D>>) -> () + 'static + Copy + std::marker::Send
    {
        for _ in 0..n_sinks {
            let (tx, rx) = mpsc::channel();
            ...
            self.transformers.push((thread::spawn(move || {
                while run_lock.lock().unwrap().get() {
                    sink_fn(&rx, own_sender.clone(), drainer.clone());
                }
            }), tx))
        }
    ...
    pub fn drain(&mut self, drainer: Sender<D>) {
